...
X=pinv(D'*D+beta*U+(2*beta+gam)*eye(L_size1))*(D'*train_data-Mx);
...
obj(iter)=norm(train_data-D*X,2)+alpha*trace(D*U*D')+beta*trace(X'*U*X)+beta*trace(X*U*X')+gam*norm(X,2);%+gams*norm(D,2);
